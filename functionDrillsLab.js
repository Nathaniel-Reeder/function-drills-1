...
let printName = (personName) => console.log(personName)
...
let compareNums = (a, b) => {
  a > b ? console.log(a + ' is the bigger number')
  : a < b ? console.log(b + ' is the bigger number')
...
let nameCheck = name => {
  let result = name === 'Steven' ? 'What is up Steven?'
...
let faveColorFinder = color => {
  let result = color === 'red' ? 'red is a great color'
...
let theEliminator = (contestants, loser) => {
  ...
  for(i = 0; i < tempContestants.length; i++){
    if(tempContestants[i] === loser){
      tempContestants.splice(i, 1)
    }
  }
  return tempContestants
}
...
let consoleFunc = stringArgument => console.log(stringArgument.toUpperCase())

consoleFunc(sampleString)
...
let emailCheck = email => {
  let emailHolder = String(email.trim())
  if (emailHolder.includes('@')) {
...
//console.log(emailCheck("    n.ash.reeder@gmail.com"   ))
...
let maxChocoFrogBuy = gold => gold / 3

let totalFrogs = maxChocoFrogBuy(4)
...
let maxChocoFrogBuy2 = gold => {
  let maxFrogs = gold / 3
  maxFrogs = Math.round(maxFrogs)
  return maxFrogs
}

let totalFrogs2 = maxChocoFrogBuy2(7)
...
let arrayAscentEval = arrayToSort => {
  let arrayToSortHolder = arrayToSort
  for (i = 0; i < arrayToSortHolder.length - 1; i++){
    if(arrayToSortHolder[i] > arrayToSortHolder[i+1]){
      return true
    } else {
      return false
    }
...
console.log(arrayAscentEval(sampleArray))
